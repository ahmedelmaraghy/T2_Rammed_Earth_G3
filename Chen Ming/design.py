...
class GlobalDesign(object):
    ...
    @classmethod
    def design_from_curve(cls, column, Layer_height, Unit_width, Unit_length, curve, h_spacing, height, default_layer_height = 40, name = "defaultName"):
        ts = curve.DivideByCount(int(round(curve.GetLength()/h_spacing*2)), True)
...
Pt_tree = th.list_to_tree(Pt_tree)
        flip_alternating = True if not curve.IsClosed else False
